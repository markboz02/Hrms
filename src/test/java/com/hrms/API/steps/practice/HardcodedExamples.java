...
public class HardcodedExamples {
	...
	static String token = "Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpYXQiOjE1OTY2NjIzODIsImlzcyI6ImxvY2FsaG9zdCIsImV4cCI6MTU5NjcwNTU4MiwidXNlcklkIjoiMTAyMiJ9.dLERblihOXLh129Igs6JbfLg6NABlkIlFYmbn0hmG1k";
	...
	public void sampleTestNotes() {
		...
		RequestSpecification getOneEmployeeRequest = given().header("Content-Type", "application/json")
				.header("Authorization", token).queryParam("employee_id", "23098A"); // .log().all();
	...
	public void aPOSTcreateEmployee() {
		 ...
		 */
		createEmployeeResponse.then().assertThat().body("Employee[0].emp_firstname", equalTo("syntaxinsan"));
	...
	public void bGETcreatedEmployee() {
		...
		Assert.assertTrue(firstName.contentEquals("syntaxinsan"));

		/** Verifying expected middle name matches actual middle name */
		Assert.assertTrue(middleName.contentEquals("syntaxyok"));

		/** Verifying expected last name matches actual last name */
		Assert.assertTrue(lastName.contentEquals("syntaxoglu"));

		/** Verifying expected birth date matches actual birth date */
		Assert.assertTrue(emp_bday.contentEquals("2000-07-18"));

		/** Verifying expected gender matches actual gender */
		Assert.assertTrue(gender.contentEquals("F"));
...
//		}
	 ...
	 */
	getUpdatedEmployeeResponse.then().assertThat().body("employee[0].emp_firstname", equalTo("syntaxInsan"));
